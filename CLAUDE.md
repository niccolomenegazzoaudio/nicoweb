...
- `user/themes/nicoweb/` — the only project-specific theme. Active via `user/config/system.yaml` (`pages.theme: nicoweb`).
  ...
  - `templates/reference.html.twig` — **special**: a self-contained replica of the old static site (Cormorant Garamond, same drone). Does not extend `partials/base.html.twig`. The route is `/reference`; the page (`user/pages/99.reference/reference.md`) sets `process: { twig: false, markdown: false }` and `visible: false`. CI explicitly checks that `/reference` contains the string `LA FEROCIA` — if you refactor this template, keep that text or update the smoke test.
