...
The header carries a single **`ò` brand mark** on the far left (`.brand` in `partials/header.html.twig`), followed by the nav items, both left-aligned (`.nav { justify-content: flex-start }`). It's rendered as a plain `o` plus a CSS-drawn `.brand__accent` bar in `--accent` (warm-tan) — same technique as the intro's symbol, just tuned to look like Space Grotesk's grave at header size (~1.6rem). We use the CSS bar instead of the precomposed `ò` glyph because we want to color **only** the accent (the font can't be partially recolored). The brand links to `/?skip=1` so clicking it from any inner page goes back to the home **without** replaying the audio intro (same mechanism as the footer Home link).
