...
Production deploy is automatic: every push to `main` runs `.github/workflows/deploy.yml`, which rsyncs to the `arch_php` host (Oracle x86_64 Arch Linux, IP `150.230.157.31`, SSH user `arch`) and runs `docker compose -f docker-compose.prod.yml up -d --build`, then **does a tar-pipe sync of the rsync'd `user/` into the running container** (the prod stack mounts named volumes per user subdirectory — `nico_pages` on `/app/user/pages`, `nico_config` on `/app/user/config`, `nico_data` on `/app/user/data`, `nico_accounts` on `/app/user/accounts` — so a plain `compose up` won't propagate repo changes to pages/themes; the workflow does `rm -rf /app/user/pages/* /app/user/themes/*` then a tar pipe of the local `user/` tree), and finally smoke-tests the public URL on the routes `/`, `/bio`, `/contatti`, `/teatro`, `/musica`, `/work-in-progress`, `/admin`. **Update the smoke test list when you change page slugs** (e.g. when about → bio, contact → contatti, the workflow's smoke test list also needs to change). The tar-pipe step excludes `user/data`, `user/accounts`, `user/config/plugins/email.yaml` so admin-runtime data and the SMTP password are preserved on prod.
...
- `user/themes/nicoweb/` — the only project-specific theme. Active via `user/config/system.yaml` (`pages.theme: nicoweb`).
  - `templates/home.html.twig` — landing page with a Web Audio drone intro: **quasi-controfase** fundamental (two sines at `freq_a` / `freq_b` = 220 / 220.4 Hz, second channel inverted via negative gain → 180° phase, the small detune avoids total cancellation in mono playback) plus a **shimmer** layer (octave + twelfth, `freq_a*2` / `freq_a*3`, modulated by a 0.3 Hz LFO with a soft 1.5s fade-in envelope). All four signals share the same lowpass sweep `filter_start → filter_end`, panning -1/+1 → 0/0 over `duration`, then fade out. All knobs (`duration`, `fade`, `freq_a`/`freq_b`, `filter_start`/`filter_end`) are read from `user/pages/01.home/home.md` frontmatter — change the page, not the template. The four-mode test panel that briefly lived on the home is gone; if you need to A/B alternative phase techniques again, restore from git history (commit `d7d67b5`).
    ...
    - **Intro is just the O.** The intro overlay shows only the symbol — `Niccolò Menegazzo` text is gone, and the O does **not** shrink on reveal. The slash is visible from page load (no fade-in). On click the slash dissolves, the accent appears, and the `.intro__hint` ("tap to get in phase" — positioned *below* the symbol, `top: calc(50% + 155px)`) fades to 0 over 3s in sync with the slash transition.
    - **Accent reveal.** When the user clicks the intro the `.symbol` gains `.active`, the slash dissolves and `.accent` fades in (opacity 0→1, 3s easing — a crossfade synced with the slash dissolve). No fire / glow / drop-shadow animation — we tried `flame-ignite` (color + filter:drop-shadow keyframes) and the resulting halo around the grave made it visually detached from the o; user wanted just the static warm-tan accent. `.accent` is the **same construction as the brand and the home hero**: an absolutely-positioned span containing the precomposed `ò` glyph, clipped via `clip-path: inset(0 0 75% 0)` so only the grave is visible above the `.o-glyph`. A small `top:6%` nudges the grave closer to the o (the font's natural ascender-to-x-height gap looks too wide at 180px). Same shape across header brand, hero name, bio body, intro symbol.
    - **Frame around the symbol.** Both the header `.brand` and the intro `.symbol` carry a 1px warm-tan frame (`var(--accent)`). The brand frame is always visible (border directly on `.brand`, `padding: 3px 8px`). The intro frame lives on `.symbol::before` as a **square 230×230px** centered on the symbol (`top:50%; left:50%; transform: translate(-50%,-50%)`), hidden until the click; then it fades in over 2s with `transition-delay: 5s` — the 5s is intentional, it's the time Nicco asked for the pulsation (`oPulse` 3.3s scale 1↔1.04 infinite) to "stabilise" before the frame appears. So timeline from click: t=0 click → 3s crossfade (slash out, accent in, hint+skip out) → t=5s frame fade-in begins → t=7s frame fully visible → t=8s overlay fade-out begins (1s hold on the full frame, per Francesco's request — DURATION governs only the audio drone, the visual exit timing is computed JS-side as `max(DURATION, 7 + CORONA_HOLD)`).
...
- prod (`docker-compose.prod.yml`): `niccolomenegazzo.com www.niccolomenegazzo.com` — Caddy auto-issues Let's Encrypt certs. Persistent state lives in named volumes — `caddy_data` and `caddy_config` for the ACME state, then a separate volume per editable user subdirectory (`nico_pages`, `nico_config`, `nico_data`, `nico_accounts`) plus runtime (`nico_cache`, `nico_logs`, `nico_tmp`, `nico_backup`) — **never** rebuild prod in a way that wipes them.
...
The header carries a single **`ò` brand mark** on the far left (`.brand` in `partials/header.html.twig`), followed by the nav items, both left-aligned (`.nav { justify-content: flex-start }`). It's drawn by rendering the precomposed `ò` glyph from Space Grotesk twice in overlap: the base text in `--text`, and a `::after` pseudo with `content: "ò"` in `--accent` clipped via `clip-path: inset(0 0 75% 0)` so only the top quarter (the grave) is visible. Result: the accent's shape is **exactly** the font's grave (not a CSS-bar approximation) but colored independently in warm-tan. The brand links to `/?skip=1` so clicking it from any inner page goes back to the home **without** replaying the audio intro (same mechanism as the footer Home link).
